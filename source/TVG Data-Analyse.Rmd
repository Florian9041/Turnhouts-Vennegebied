...
data_TVG_filtered <- data_TVG %>%
  arrange(FieldSamplingDate) %>%
  mutate(
    is_post_2023 = year(FieldSamplingDate) >= 2023,
    prev_date = lag(FieldSamplingDate),
    next_date = lead(FieldSamplingDate),
    prev_diff = interval(prev_date, FieldSamplingDate) %/% months(1),
    next_diff = interval(FieldSamplingDate, next_date) %/% months(1)
  ) %>%
  filter(
    !is_post_2023 |
    (is_post_2023 & 
     ((is.na(prev_diff) | prev_diff <= 4) &
      (is.na(next_diff) | next_diff <= 4)))
  ) %>%
  select(-is_post_2023, -prev_date, -next_date, -prev_diff, -next_diff)
...
generate_plot <- function(codeplas_value, aqua_component, watertype_thresholds, y_label = NULL) {
  ...
  watertype <- unique(filtered_data$watertype)
  threshold_min <- watertype_thresholds[[watertype]]$min %||% NA
  threshold_max <- watertype_thresholds[[watertype]]$max %||% NA
  ...
  plot <- filtered_data %>%
    ...
    geom_line(linewidth = 1, color = "orange") +
    geom_point(color = "orange") +
    geom_hline(yintercept = threshold_min, linetype = "dashed", color = "blue", size = 1) +
    geom_hline(yintercept = threshold_max, linetype = "dashed", color = "red", size = 1) +
    labs(x = "Datum staalname",
         y = y_axis_label)
...
plots_pH <- generate_multiple_plots(reeks_2022_2024, "pH", grenswaarden_pH, "pH")
