...
# concentratie
data_TVG %>%
  filter(AquaComponent %in% "O2_veld" & codeplas %in% reeks_2024) %>%
  group_by(codeplas) %>%
  summarise(O2_10p = quantile(ResultCalc,probs = 0.1, na.rm = TRUE)) %>%
  write.csv2(file = "./exports/O2_conc_2024.csv")
data_TVG %>%
  filter(AquaComponent %in% "O2_veld" & codeplas %in% reeks_2022_2023) %>%
  group_by(codeplas) %>%
  summarise(O2_10p = quantile(ResultCalc,probs = 0.1, na.rm = TRUE)) %>%
  write.csv(file = "./exports/O2_conc_2022-23.csv")

# verzadiging
data_TVG %>%
  filter(AquaComponent %in% "O2verz" & codeplas %in% reeks_2024) %>%
  group_by(codeplas) %>%
  summarise(O2_verz_max = max(ResultCalc, na.rm = TRUE)) %>%
  write.csv(file = "./exports/O2_verz_2024.csv")
data_TVG %>%
  filter(AquaComponent %in% "O2verz" & codeplas %in% reeks_2022_2023) %>%
  group_by(codeplas) %>%
  summarise(O2_verz_max = max(ResultCalc, na.rm = TRUE)) %>%
  write.csv(file = "./exports/O2_verz_2022-23.csv")
```
...
data_TVG %>%
  filter(AquaComponent %in% "T_veld" & codeplas %in% reeks_2022_2023) %>%
  group_by(codeplas) %>%
  summarise(Temp_max = max(ResultCalc, na.rm = TRUE)) %>%
  write.csv(file = "./exports/Temp_veld22_23.csv")
```
